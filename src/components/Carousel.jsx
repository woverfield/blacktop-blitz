  ...
  return (
    <div className="carousel container mx-auto">
      ...
      {slides.slides.map((item, idx) => {
        return (
          <Link to={(idx + 1).toString()} >
            <h1 className={slide === idx ? "carousel-heading bg-white rounded-md p-3 px-10 text-3xl" : "hidden"}>{idx + 1} vs. {idx + 1}</h1>
            <button className={slide === idx ? "size-btn" : "size-btn hidden"}>
              <img
                src={item.src}
                alt={item.alt}
                key={idx}
